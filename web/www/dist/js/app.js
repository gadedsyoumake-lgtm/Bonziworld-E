                ...
                },
                {
                    ...
                    value: function (text, say, allowHtml) {
                        ...
                        if(settings.notifications.value === true && LoggedIn === true) {try {new Notification("Room ID: " + Room_ID, { body: date + " | " + this.userPublic.name + ": " + text, icon: "./img/agents/__closeup/" + this.userPublic.color + ".png" })} catch {}};
                                                var toscroll = document.getElementById("chat_log_list").scrollHeight - document.getElementById("chat_log_list").scrollTop < 2;
                                                var linkifiedText = linkify(text);
                                                document.getElementById("chat_log_list").innerHTML += "<ul><li class=\"bonzi-message cl-msg ng-scope bonzi-event\" id=\"cl-msg-"+self.id+"\"><span class=\"timestamp ng-binding\"><small style=\"font-size:11px;font-weight:normal;\">"+date+"</small></span> <span class=\"sep tn-sep\"> | </span><span class=\"bonzi-name ng-isolate-scope\"><span class=\"event-source ng-binding ng-scope\"><font color='"+getBonziHEXColor(this.userPublic.color)+"'>"+this.userPublic.name+"</font></span></span><span class=\"sep bn-sep\">: </span><span class=\"body ng-binding ng-scope\" style=\"color:#dcdcdc;\">"+linkifiedText+"</span><button class=\"copy-log-btn\" onclick=\"navigator.clipboard.writeText(this.closest('.cl-msg').innerText.replace(/Copy/, '').trim());this.textContent='Copied!';setTimeout(()=>this.textContent='Copy',2000);\" style=\"background:#555;color:#dcdcdc;border:1px solid #777;padding:4px 8px;border-radius:3px;margin-left:10px;cursor:pointer;font-size:11px;font-family:monospace;\">Copy</button></li></ul>";
...
function linkify(text) {
    return text.replace(/(https?:\/\/([-\w\.]+)+(:\d+)?(\/([\w\/_\.]*(\?\S+)?)?)?)/gi, "<a href='$1' target='_blank' style='color:#4a9eff;text-decoration:underline;cursor:pointer;'>$1</a>");
}
